...
func GetApi( url_option string ) (result *http.Response, err error){
    values := url.Values{}
    values.Add( "keyid", CONSUMER_KEY )
    resp, err := http.Get( URL + "?" + values.Encode() + url_option )
    return resp, err
}

func ReturnStruct( url_option string ) (result AutoGenerated, err error ) {
	autogenerated := new( AutoGenerated )
    tmp, err := GetApi( url_option )
